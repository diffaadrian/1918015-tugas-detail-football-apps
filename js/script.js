const ApiKey = "223efd7915a942dba4ed3774f3622b87";
...
function getListTeams() {
    ...
    fetch(teamEndPoin, fetchHeader)
        ...
        .then(resJson => {
            ...
            const lengkap = document.querySelectorAll('.secondary-content');
            lengkap.forEach(btn => {
                btn.onclick = (event) => {
                    showInfoTeam(event.target.dataset.id);
                }
            })
        }).catch(err => {
            console.error(err);
...
function showInfoTeam(id){
    let url = baseUrl + "teams/" + id;
    const detailteam = url;
    title.innerHTML = " Ini merupakan tampilan detail dari tim ";

    fetch(detailteam, fetchHeader)
        .then(response => response.json())
        .then(resJson => {
            console.log(resJson);
            let squad = "";
            resJson.squad.map((player) => {
                squad += `
                <ol>
                    <li>Nama : ${player.name}</li>
                    <li>Posisi Player : ${player.position}</li>
                    <li>Kebangsaan : ${player.nationality}</li>
                    </ol>
                    <hr>
                `;
            });
            ...
            if (resJson) {
                ...
                <li class="collection-item avatar">
                    ...
                    <span class="title">${resJson.name}</span>
                    <p>Atau biasa disebut dengan ${resJson.shortName} <br>
                       Berdiri sejak tahun: ${resJson.founded} <br>
                       Bermarkas di Stadion: ${resJson.venue}<br>
